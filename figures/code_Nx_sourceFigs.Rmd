...
## fix proteomics ortholog annotation for Fig. 5b and ED 5a
...
resmut <- fread("~/Documents/EMBL/Nx/proteomics_strains_Marco/results_with_pangenome.tsv") %>%
  dplyr::mutate(species = case_when(Strain_treatment %in% c("PRZS", "MS02S","MS25S","MS274R","MS299R","MS311R","MS366R","MS386R", "MS493R", "MS530R","MS55S") ~ "K. pneumoniae",
                                    ...
                                    TRUE ~ "E. coli"),
                og1 = case_when(og %in% c("ompc~~~ompk36", "wp_000865543.1") | protein_ID == "P06996" | description %in% c("Outer membrane protein C","outer membrane protein C") ~ "OmpC",
                                description %in% c("Outer membrane protein Slp") ~ "Slp",
                                gene_name == "oprC" ~"OprC",
                                og %in% c("ompW") | description %in% c("outer membrane protein ompW", "Outer membrane protein OmpW") ~ "OmpW",
                                description %in% c("2-hydroxy-3-oxopropionate reductase") ~ "GarR",
                                description %in% c("2,3-dehydroadipyl-CoA hydratase") ~ "PaaF",
                                description %in% c("4-aminobutyrate aminotransferase GabT") ~ "GabT",
                                ...
                                og %in% c("bepf~~~oqxA", "adeF") | gene_name == "bpeE" ~ "BpeE/AdeF/OqxA/BepF",
                                ...
                                og %in% c("basJ","entC") | description %in% c("Isochorismate synthase EntC") ~ "EntC",
                                description %in% c("Low-affinity putrescine importer PlaP") ~ "PlaP",
                                og %in% c("wp_000973666.1", "lamB") | gene_name == "lamB" | protein_ID == "P02943" | description %in% c("maltoporin", "Maltoporin") ~ "LamB",
                                ...
                                og %in% c("soxS_1", "wp_000091198.1") | protein_ID == "P37340" | description %in% c("multiple antibiotic resistance protein marA", "Multiple antibiotic resistance protein MarA") ~ "MarA",
                                ...
                                og %in% c("ompC~~~ompK36", "wp_000865543.1") | protein_ID == "P06996" | description %in% c("outer membrane protein C", "Outer membrane protein C") ~ "OmpC",
                                ...
                                description %in% c("Succinate-semialdehyde dehydrogenase [NAD(P)+]",
                                                   "Succinate-semialdehyde dehydrogenase [NADP(+)] 1","Succinate-semialdehyde dehydrogenase [NADP(+)] GabD") ~ "GabD1",
                                og %in% c("emrR", "mprA") | description %in% c("Transcriptional repressor MprA") ~ "EmrR",
                                ...
                                og == "yciE" ~ "YciE",
                                 og %in% c("oqxB3", "adeG") | gene_name == "bpeF"~ "BpeF/AdeG/OqxB",
                         og %in% c("bepA_1", "bepA_2") ~ "BepA",
                         og %in% c("emrA_1", "emrA_3") ~ "EmrA",
                         og %in% c("adeB","acrB_1") ~ "AcrB",
                         og %in% c("acrR","acrR~~~acrR_1","wp_000101737.1") ~ "AcrR",
                         og %in% c("adeI","acrA") ~ "AcrA",
                        description == "transcriptional regulatory protein ompR" ~ "OmpR",
                         description %in% c("outer membrane protein tolC", "Outer membrane protein TolC") ~ "TolC",
                         og %in% c("group_2715","pgaA~~~hmsH") ~ "PgaA",
                         og %in% c("scrY_1") ~ "ScrY",
                         og %in% c("wp_000737271.1") ~ "OmpD",
                         og %in% c("ompC~~~ompN_1") | protein_ID == "P77747" | description == "outer membrane protein N" ~ "OmpN",
                        og == "wp_000977927.1" | protein_ID == "P02931" | description == "outer membrane protein F" ~ "OmpF",
                        description == "Outer membrane protein Omp38" ~ "Omp38",
                         og %in% c("ompX") | description %in% c("outer membrane protein ompX", "Outer membrane protein OmpX") ~ "OmpX",
                         description == "Outer membrane protein YedS" ~ "YedS",
                        description == "Outer membrane protein A" | protein_ID == "P0A910" ~ "OmpA",
                         og %in% c("oprB_1") | protein_ID == "D0CDE3" ~ "OprB",
                         og %in% c("oprD", "group_3145") ~ "OprD",
                        og %in% c("mdtQ~~~cusC_1", "adeK") | protein_ID == "D0CDQ0"  ~ "OprM",
                        og %in% c("mdtK", "wp_001174963.1") | protein_ID == "P37340" ~ "MdtK",
                        TRUE ~ og)) %>%
  #mutate(og1 = capitalize_first_and_last(og1)) %>%
  capitalize_first_last()
...
View(toplot %>% group_by(Species) %>%
  dplyr::summarise(meanMIC=mean(MIC),
            medianmic=median(MIC)))
...
# per family
toplot=toplot %>%
  mutate(family = case_when(Species %in% c("Neisseria gonorrhoeae") ~ "Neisseriales",
            Species %in% c("Achromobacter xylosoxidans","Burkholderia cenocepacia") ~ "Burkholderiales",
            Species %in% c("Stenotrophomonas maltophilia") ~ "Xantomonadales",
...
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_2a_EDFig.3a.xlsx"))
...
# remove dead space for Bliss cumulative score
...
## Fig. 3b - chemical genetics - SUPP TABLE 4
...
stat.test = stat.test %>%
 ...
 ggplot(towrite, aes(x = condition, y = mean_mic, color = condition)) +
  geom_bar(aes(fill = condition),stat = "identity", position = position_dodge(0.8), width = 0.7)+
   geom_point(aes(x = condition,y=`MIC (µg/ml)`), pch=21,fill='grey90', size=2, show.legend=F,position = position_jitterdodge(dodge.width = 0.6, jitter.width = 0.9,jitter.height = 0.4)) +
...
up = fread("~/Documents/EMBL/Nx/Nx_paper/REVISION/nitroxoline_2024/GO_enrichment/outputdir/GOenrich_Keio_Up.txt", sep = "\t")
down = fread("~/Documents/EMBL/Nx/Nx_paper/REVISION/nitroxoline_2024/GO_enrichment/outputdir/GOenrich_Keio_Down.txt", sep = "\t")
...
# write source data
towrite = toplot %>%
  select(-c(hit_genes,freq_back_minus_fore,freq_fore_non_GOid,freq_back_non_GOid,num_genes_back,colour,colour2,full_name)) %>%
  rename(effect=source,
         annotation=annot) %>%
  mutate(effect=gsub(effect,pattern="\n",replacement=" "))

write_xlsx(towrite, paste0(sourcedatadir,"source_EDFig.4c.xlsx"))
...
test = toplot %>%
              filter(source == "increased fitness")
...
write_xlsx(toplot, paste0(sourcedatadir,"source_Fig.S4.xlsx"))
...
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"Fig.S5_growthCurves_LptD.xlsx"))
...
#toplot = read.table(paste0(inputdir,"remsut_conservation_annotated_forplotting.txt"), sep="\t", header = T)
...
# bla = filter(toplot, Condition_treatment == "Nx" & !(Strain_treatment %in% strainstorem))
# 
# # add new strain designation
# # strain_key = readxl::read_xlsx(paste0(inputdir,"new_designation_240525_MT_EC.xlsx")) %>%
# #   clean_names()
# strain_key = readxl::read_xlsx(paste0(inputdir,"SuppTable1_290601.xlsx"),skip=1) %>%
#   clean_names() %>%
#   filter(!(genotype %in% c("K. pneumoniae PRZ pKP2442 mcr-1", "K. pneumoniae PRZ pTOPO", "K. pneumoniae PRZ pTOPO mcr-1"))) %>%
#   mutate(name_used = sapply(strsplit(as.character(name_used), " "),'[',3))
# 
# bla =   bla %>%
#   mutate(name_used = sub("[RS]$", "",complete_name),
#          name_used = sapply(strsplit(as.character(name_used), " "),'[',3)) %>%
#   inner_join(strain_key, by="name_used")
# 
# bla = bla %>% 
#   dplyr::select(og1, conservation, cross_species_conservation,protein_ID,gene_name,description, Condition_treatment,Condition_control, new_designation, logFC,AveExpr,t,P.Value,adj.P.Val,B,hit,species) %>%
#   dplyr::rename(Strain_treatment=new_designation,
#                 og=og1)
