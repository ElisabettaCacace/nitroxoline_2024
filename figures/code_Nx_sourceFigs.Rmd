...
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_Fig.1d.xlsx")) %>%
  mutate(condition = factor(condition, levels=c("before treatment", "DMSO", "nitroxoline (5 µg/ml)")),
...
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_Fig.1e_SuppFig.2b.xlsx")) %>%
  filter(`Nitroxoline concentration (µg/ml)` != 0)
...
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_Fig.2a_SuppFig.3a.xlsx"))
...
test = toplot %>%
  #don't include empty vector control in main fig, but in Supplementary Fig. 4 (full growth curves)
...
p1 <-   ggplot(filter(test, row==2), aes(x = as.factor(`Colistin concentration (µg/ml)`), y = meanOD, group = interaction(strain, nitroxoline), colour = nitroxoline)) +
...
p2 <- ggplot(filter(test, row==3), aes(x = as.factor(`Colistin concentration (µg/ml)`), y = meanOD, group = interaction(strain, nitroxoline), colour = nitroxoline)) +
   geom_line(linewidth=1,aes(linetype=resistance)) +
    geom_errorbar(aes(ymin = meanOD-sem, ymax = meanOD + sem), width = 3,size=1.3, position=position_dodge(0.2)) +
   geom_point(aes(shape=strain, color = nitroxoline),fill="white",stroke=1.5,size = 5.5,position=position_dodge(0.1)) +
  scale_color_manual(labels = c("+ Nitroxoline 0.75 µg/ml", "- Nitroxoline"), values = c("turquoise4","darkgrey")) +
...
p3 <- ggplot(filter(test, row==5), aes(x = as.factor(`Colistin concentration (µg/ml)`), y = meanOD, group = interaction(strain, nitroxoline), colour = nitroxoline)) +
   geom_line(linewidth=1,aes(linetype=resistance)) +
...
p4 <- ggplot(filter(test, row==6), aes(x = as.factor(`Colistin concentration (µg/ml)`), y = meanOD, group = interaction(strain, nitroxoline), colour = nitroxoline)) +
   geom_line(linewidth=1,aes(linetype=resistance)) +
    geom_errorbar(aes(ymin = meanOD-sem, ymax = meanOD + sem), width = 2,size=1.3, position=position_dodge(0.2)) +
   geom_point(aes(shape=strain, color = nitroxoline),fill="white",stroke=1.5,size = 5.5,position=position_dodge(0.1)) +
  scale_color_manual(labels = c("+ Nitroxoline 0.75 µg/ml", "- Nitroxoline"), values = c("turquoise4","darkgrey")) +
...
p5 <- ggplot(filter(test, row==7), aes(x = as.factor(`Colistin concentration (µg/ml)`), y = meanOD, group = interaction(strain, nitroxoline), colour = nitroxoline)) +
   geom_line(linewidth=1,aes(linetype=resistance)) +
...
p6 <- ggplot(filter(test, row==8), aes(x = as.factor(`Colistin concentration (µg/ml)`), y = meanOD, group = interaction(strain, nitroxoline), colour = nitroxoline)) +
   geom_line(linewidth=1,aes(linetype=resistance)) +
...
ggplot(toplot, aes(x = time_after_infection_h, y = mean, color = condition, group = condition))+
 ...
 scale_color_manual(values = c('#009999','#FF6699','#FF6600', '#666666'),
                       breaks = c("0,1 µg/mL nx + 0,1 µg/ml CT", "0,1 µg/ml CT", "0,1 µg/ml Nx", "NaOAc"),
...
colours = data.frame("Condition" = c("PolymyxinB_10","EDTA_0.4", "Nitroxoline_0.45", "Nitroxoline_0.9", "Nitroxoline_0.225", "Chloramphenicol_8", "nodrug_0"),
...
names(jColors) <- colours$Condition
 ...
 ggerrorplot(toplot, x = "Condition", y = "NPN_fluorescence", error.plot="errorbar",
          order = c("PolymyxinB_10","EDTA_0.4", "Nitroxoline_0.45", "Nitroxoline_0.9", "Nitroxoline_0.225",  "Chloramphenicol_8", "nodrug_0"),
  add = "se") +
   stat_summary(
    geom = "point",
    shape = 95,
...
```{r}
toplot =readxl::read_xlsx(paste0(sourcedatadir,"source_Fig.4abc_SuppFig5bf.xlsx"))
...
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_Fig.5b_SuppFig.6b.xlsx"))
...
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_Fig.5c.xlsx")) %>% 
  mutate(strain_new = case_when(strain %in% c("4_R1","4_R1 + oqxR-wt","4_R1 + pTOPO") ~ "4_R1 + wt ~ italic('oqxR')",
                             strain %in% c("1_R4","1_R4 + emrR-wt","1_R4 + pTOPO") ~ "1_R4 + wt ~ italic('emrR')"),
...
stat.test <- toplot %>%
  ...
  wilcox_test(`MIC (µg/ml)` ~ condition, p.adjust.method = "BH") %>%
...
stat.test = stat.test %>% mutate(y.position = log2(y.position))
...
stat.test = stat.test %>%
 ...
 ggplot(toplot, aes(x = condition, y = mean_mic, color = condition)) +
  geom_bar(aes(fill = condition),stat = "identity", position = position_dodge(0.8), width = 0.7)+
   geom_point(aes(x = condition,y=`MIC (µg/ml)`), pch=21,fill='grey90', size=2, show.legend=F,position = position_jitterdodge(dodge.width = 0.6, jitter.width = 0.9,jitter.height = 0.1)) +
...
#  scale_y_continuous(breaks = c(0,2,4,6,8,10), labels= c(0,4,16,64,256,1024)) +
    scale_y_continuous(breaks= c(0,4,16,64,256,1024), trans = "log2") +
...
ggplot(toplot,aes(location_aa_number,`MIC (µg/ml)`,color=Isolate_type,label=Protein_Change))+
  geom_segment(aes(y=0,xend=location_aa_number,yend=`MIC (µg/ml)`)) +
  geom_point(aes(size=Mutation_occurrences, shape=Mutation_Type, color=Isolate_type)) +
  geom_text_repel(data=toplot %>%
                    distinct(location_aa_number,`MIC (µg/ml)`, Protein_Change,Isolate_type,Mutation_occurrences),
  ...
  labs(y= "MIC (µg/mL)",size="Mutation occurrencies", title = "OqxR", shape = "Mutation type")+
  theme_bw() +
...
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_Fig.5e_SuppFig.6g.xlsx")) %>%
...
p <- ggerrorplot(toplot %>% filter(row==1),
                 x = "condition", y = "`MIC(µg/ml)`", error.plot="errorbar",
                 order = c("no inhibitor","+ 100 µg/ml NMP", "+ 25 µg/ml PAβN"), add = "se") +
...
cairo_pdf(paste0(plotdir,"Fig.5e_EPI.pdf"), width =7,height=6)
...
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.1a.xlsx")) %>%
  mutate(Species_italics = case_when(Species == "Salmonella spp." ~ "*Salmonella* spp.",
                                     Species == "Shigella spp." ~ "*Shigella* spp.",
                                    TRUE ~ paste0("*", Species, "* ")),
         MIC=as.numeric(MIC),
...
pdf(paste0(plotdir,"SuppFig.1a_Disk_diffusion.pdf"), width=9, height=6)
ggplot(toplot,aes (x = as.factor(MIC), Species_label)) +
    geom_tile(aes(fill = number_strains_at_MIC, color="black"), colour = "black", size = 0.25) +   
...
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.1b.xlsx")) %>%
  mutate(Species_italics = case_when(Species == "Salmonella spp." ~ "*Salmonella* spp.",
                                     Species == "Shigella spp." ~ "*Shigella* spp.",
                                    TRUE ~ paste0("*", Species, "* ")),
         MIC=as.numeric(MIC),
...
pdf(paste0(plotdir,"SuppFig.1b_agar_microdilutions.pdf"), width=4, height=6)
ggplot(toplot,aes (x = as.factor(MIC), Species_label)) +
    geom_tile(aes(fill = number_strains_at_MIC, color="black"), colour = "black", size = 0.25) +   
    geom_text(aes(label=number_strains_at_MIC),size=3.3) +
...
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.1c.xlsx")) %>%
  mutate(newspec = paste0("italic('", toplot$Species, "')"))


pdf(paste0(plotdir,"SuppFig.1c_eucast_broth_concordance.pdf"),width = 4, height =4)
...
### S1d - correlation between disk and agar

```{r}

toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.1d.xlsx")) %>%
  mutate(newspec = paste0("italic('", Species, "')"))


pdf(paste0(plotdir, "SuppFig.1d_corr_disk_agar.pdf"), width = 5.5, height = 5.5)
...
### S1e - correlation broth and agar

```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.1e.xlsx")) %>%
  mutate(newspec = paste0("italic('", Species, "')"))


pdf(paste0(plotdir, "SuppFig.1e_corr_broth_agar.pdf"), width = 5.5, height = 5.5)
...
### S1f - correlation between disk and broth

```{r}

toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.1f.xlsx")) %>%
  mutate(newspec = paste0("italic('", Species, "')"))


pdf(paste0(plotdir, "SuppFig.1f_corr_disk_broth.pdf"), width = 5.5, height = 5.5)
...
### S2a - E. coli killing
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.2a.xlsx"))


pdf(paste0(plotdir,"SuppFig.2a_Ecoli_killing.pdf"), width = 5.8, height=5)

toplot$`Nitroxoline concentration (µg/ml)` <- factor(toplot$`Nitroxoline concentration (µg/ml)`, levels=c('0', '2', '4', '8', '16', '32','64'))

my_palette = brewer.pal("Oranges",n = 9)[3:9]

ggplot(toplot, aes(as.factor(`Time (h)`),mean, group = `Nitroxoline concentration (µg/ml)`, color = `Nitroxoline concentration (µg/ml)`)) +
  geom_point(size=0.8) +
  scale_y_log10(labels= trans_format(log10, math_format(10^.x)),
                breaks =trans_breaks(log10, function(x) 10^x, 10)) +
...
pdf(paste0(plotdir,"SuppFig.2b_Abau_killing_control.pdf"), width = 5.8, height=5)
...
## Supp. Fig. 3
### S3a - all checkerboards

```{r}
allCBs = readxl::read_xlsx(paste0(sourcedatadir,"source_Fig.2a_SuppFig.3a.xlsx")) %>%
...
pdf(paste0(plotdir, "SuppFig.3a_CB_heatmaps_hits.pdf"),  width=10, height=15)
layout <-"BBBBB
          AAAAA
          AAAAA
          AAAAA
...
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.5a.xlsx")) %>%
...
pdf(paste0(plotdir, "SuppFig.5a_GObarplots.pdf"), width = 12, height = 12)
design <- "
111333
111333
222444
...
pdf(paste0(plotdir,"SuppFig.5b_thermal_profile_envstress.pdf"), width = 7, height = 3.8)
...
### S5c - GO enrichment hits from chemical genetics


```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.5c.xlsx")) %>%
...
pdf(paste0(plotdir, "SuppFig.5c_GO_Keio.pdf"), width = 12, height = 6)
...
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.5d.xlsx")) %>%
  mutate(concentration= as.factor(`Nitroxoline conc (µg/ml)`))
...
pdf(paste0(plotdir, "SuppFig.5d_dose_response_lptD.pdf"), width = 3.2, height = 3.5)
...
pdf(paste0(plotdir, "SuppFig.5f_thermal_profile_iron.pdf"), width = 5.8, height = 4)
...
### S6a - volcano R strains
```{r}

toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.6a.xlsx"))
...
pdf(paste0(plotdir, "SuppFig.6a_resistant_strains_volcano.pdf"), width = 8, height = 7.6)
...
### S6b - clustered MS - only significant genes, resistant strains
...
pdf(paste0(plotdir,"SuppFig.6b_clustered_proteomics.pdf"), width=6, height=30)
...
# read data with MIC and isolate type info
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.6c.xlsx"))
...
pdf(paste0(plotdir,"SuppFig.6c_EmrR_mutplot.pdf"), width=4.5, height=3.5)

ggplot(toplot,aes(location_aa_number,`MIC (µg/ml)`,color=Isolate_type,label=Protein_Change))+
  geom_segment(aes(y=0,xend=location_aa_number,yend=`MIC (µg/ml)`)) +
  geom_point(aes(size=Mutation_occurrences, shape=Mutation_Type, color=Isolate_type)) +
  geom_text_repel(data=toplot %>%
                    distinct(location_aa_number,`MIC (µg/ml)`, Protein_Change,Isolate_type),
  ...
  labs(y= "MIC (µg/ml)",size="Total mutation\noccurrencies", shape = "Mutation type")+
  theme_bw() +
  theme(
...
# read data with MIC and isolate type info
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.6e.xlsx"))
...
ggplot(toplot,aes(location_aa_number,`MIC (µg/ml)`,color=Isolate_type,label=Protein_Change))+
 ...
 annotate("text", x = 29, y = 2.3, label ="HTH marR-type", size = 4,color="white",fontface="bold") +
  ...
  labs(y= "MIC (µg/mL)",size="Mutation occurrencies", title = "AdeL")+
  theme_bw() +
  theme(plot.title=element_blank(),
       legend.position = "top",
       legend.direction="vertical",
...
# read data with MIC and isolate type info
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.6f.xlsx"))
...
pdf(paste0(plotdir,"SuppFig.6f_acrR_mutplot.pdf"),width=4.5, height=4)
...
ggplot(toplot,aes(location_aa_number,`MIC (µg/ml)`,color=Isolate_type,label=Protein_Change))+
  ...
  labs(y= "MIC (µg/ml)",size="Mutation occurrencies", title = "AcrR")+
  theme_bw() +
  theme(plot.title=element_blank(),
       legend.position = "top",
       legend.direction="vertical",
...
p <- ggerrorplot(toplot %>% filter(row==1), x = "condition", y = "`MIC(µg/ml)`", error.plot="errorbar",
...
p2 <- ggerrorplot(toplot %>% filter(row==2), x = "condition", y = "`MIC(µg/ml)`", error.plot="errorbar",
          order = c("no inhibitor","+ 100 µg/ml NMP", "+ 25 µg/ml PAβN"),
  add = "se") +
 #  stat_summary(geom = "point",shape = 95,size = 12,col = "black",fun.y = "mean") +
  facet_wrap(~ newspec, nrow=1, labeller = label_parsed) +
...
cairo_pdf(paste0(plotdir,"SuppFig.6g_effluxpump.pdf"), width =7,height=4)
...
## Supp. Fig. 7 - GO resistant strains

```{r}

final = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.7.xslx"))
...
pdf(paste0(plotdir, "SuppFig.7_GO_enrich_proteomics_ResistantStrains.pdf"), width = 12, height = 15)
design <- "
4455#
4455#
4455#
...
## Supp. Fig. 8 - GO sensitive strains

```{r}

final = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.8.xslx"))
...
pdf(paste0(plotdir, "SuppFig.8_GO_enrich_SensitiveStrains.pdf"), width = 12, height = 17)
...
## Supp. Fig. 9 - MTT toxicity

```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_SuppFig.9.xlsx"))


toplot$condition = factor(toplot$condition, levels = c("0.5", "1", "2","5","10","20","50","0.1%\nTriton", "RPMI"), ordered = TRUE)


pdf(paste0(plotdir,"SuppFig.9_MTT_assay.pdf"),width =4,height=2)
...
# ADDITIONAL FIGURES (only in git)

## Fig. 1 - checkerboard replicates
...
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_additionalFig.2.xlsx"))
...
```{r}
toplot = readxl::read_xlsx(paste0(sourcedatadir,"source_additionalFig.3.xlsx"))

pdf(paste0(plotdir,"fullgrowthcurves_lptD.pdf"),width = 10, height=2)
ggplot(toplot%>%
...
## Fig. 4 - resp chain thermal profiles
...
pdf(paste0(plotdir, "thermal_profile_respiratory_chain.pdf"), width = 14, height = 7)

cowplot::plot_grid(a, b,c,d,e,f,g,h,i,j,k, ncol = 5)
...
## Fig. 5 - proteomics sensitive strains
### 5a - volcano sensitive strains proteomics
...
pdf(paste0(plotdir, "volcano_sensitive_strains.pdf"), width = 10, height = 1.9)
...
###5b - clustered hm only hits sensitive proteomics
...
pdf(paste0(plotdir,"clustered_proteomics_onlySensitiveStrains.pdf"), width=6, height=8)
...
Other supplementary tables are generated earlier when plotting figures. Please search for "table" in this code.
